#!/usr/bin/env python3
"""Recompute every figure in the "Governing the Machine" results table from a
repository's own history. (The paper is published separately; this script is the
definition of the numbers it quotes, which is why it lives with the code.)
...
Definitions, stated once so they cannot drift from the prose:
  ...
  window            [start, end] inclusive, by merge date in UTC. The paper's
                    ...
                    calendar boundaries; that choice is argued in §6 and is an
                    input here, not an assumption baked in.
...
def issue_stats(issues: list[dict], label: str = "auto-filed") -> dict:
    auto = [i for i in issues
            if any(lb.get("name") == label for lb in i.get("labels") or [])]
    closed = [i for i in auto if i.get("closedAt")]
    hours = sorted((_ts(i["closedAt"]) - _ts(i["createdAt"])).total_seconds() / 3600
                   for i in closed)
    median = None
    if hours:
        mid = len(hours) // 2
    ...
    return {
        ...
        "median_hours_to_close": round(median, 1) if median is not None else None,
    }
...
def main() -> int:
    ...
    issues = _gh_json(args.repo, "issue", "number,createdAt,closedAt,labels")
    ...
    print("\nqueue (whole repository, not windowed):")
    for k, v in issue_stats(issues).items():
        print(f"  {k:24} {v}")
    return 0
